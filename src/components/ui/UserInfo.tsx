...
export default async function UserInfo({ user, isMe = false }: { user: UserType, isMe: boolean }) {
    const resFollowers = await fetch(
        `${process.env.BACKEND_URL}/follow/countFollowers?userId=${user.id}`,
    ...
    const resFollowing = await fetch(
        `${process.env.BACKEND_URL}/follow/countFollowing?userId=${user.id}`,
        { cache: 'no-store' }
    );
    ...
    const res = await fetch(`${process.env.BACKEND_URL}/follow/user?followingId=${user.id}`, {
